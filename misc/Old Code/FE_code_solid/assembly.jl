...
function assembleElement!(E, element, Ke, fe, quad::QuadratureRules, shape::ShapeFunctions)
	# Initialise
	nEleNodes = size(Ke)[1]
	for i = 1:nEleNodes
		fe[i] = 0.0
	...
	for q = 1:quad.nQuadPoints
		...
		for i = 1:nEleNodes
			fe[i] += 0
			for j = 1:nEleNodes
				Ke[i,j] += integrator * B[:,i]' * getDmatrix(E) * B[:,j]
			end
		end
	end
end;

function globalAssembly(E, ncon, coords, quad::QuadratureRules, shape::ShapeFunctions)
	...
	for e = 1:nElements
		...
		assembleElement!(E, elePoints, Ke, fe, quad, shape)
