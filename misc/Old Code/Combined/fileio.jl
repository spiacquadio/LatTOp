...
function export2vtk(filepath, points, ncon, sol = 0.0, dofs = 1, cellData = 0.0)
	...
	if length(sol) > 1
		...
		write(file, "\t\t\t\t<DataArray type=\"Float64\" Name=\"u\" NumberOfComponents=\"2\" format=\"ascii\">\n");
		for i = 1:nNodes
			if dofs == 1
	...
	if length(cellData) > 1
		...
		for i = 1:nElements
			write(file, @sprintf("\t\t\t\t\t%f %f %f\n", cellData[i,1],cellData[i,2],cellData[i,3]));
		end
		write(file, "\t\t\t\t</DataArray>\n");
		write(file, "\t\t\t</CellData>\n");
	end
